...
class SaleOrderValidation(models.Model):
    ...
    def sale_whatsapp(self):
        ...
        if not record_phone:
            view = self.env.ref('odoo_whatsapp_integration1.warn_message_wizard')
        ...
        if not record_phone[0] == "+":
            ...
            return {'type': 'ir.actions.act_window',
                    'name': _('Gửi tin nhắn Zalo'),
                    'res_model': 'whatsapp.wizard',
                    'target': 'new',
                    'view_mode': 'form',
                    'view_type': 'form',
                    'context': {
                        'default_template_id': self.env.ref('odoo_whatsapp_integration1.whatsapp_sales_template').id},
                    }
    ...
    def multi_sms(self):
        ...
        if cust_check:
            ...
            form_id = self.env.ref('odoo_whatsapp_integration1.whatsapp_multiple_message_wizard_form').id
            product_all = []
